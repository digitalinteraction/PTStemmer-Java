...
public abstract class XMLUtils {
	...
	public static Collection<Element> getChilds(Element e) {
		final List<Element> childs = new ArrayList<Element>();
		if (e == null) {
			return childs;
		}

		final NodeList cnodes = e.getChildNodes();
		for (int i = 0; i < cnodes.getLength(); i++) {
			final Node cnode = cnodes.item(i);
			if (cnode instanceof Element) {
				final Element child = (Element) cnode;
				childs.add(child);
			}
		}
		return childs;
	...
	public static void setProperty(SuffixTree<?> tree, String propertyName, Integer defaultValue, Element element) {
		try {
			final int value = Integer.parseInt(element.getAttribute(propertyName));
