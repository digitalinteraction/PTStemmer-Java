...
public class PorterStemmer extends Stemmer {
	...
	private final static HashSet<Character> vowels = new HashSet<Character>(
			Arrays.asList('a', 'e', 'i', 'o', 'u', 'á', 'é', 'í', 'ó', 'ú', 'â', 'ê', 'ô'));

	private final static SuffixTree<Boolean> suffix1 = new SuffixTree<Boolean>(true, "amentos", "imentos", "amento",
			"imento", "adoras", "adores", "aço~es", "ismos", "istas", "adora", "aça~o", "antes", "ância", "ezas",
			"icos", "icas", "ismo", "ável", "ível", "ista", "osos", "osas", "ador", "ante", "eza", "ico", "ica", "oso",
			"osa");
	...
	private final static SuffixTree<Boolean> suffixv = new SuffixTree<Boolean>(true, "aríamos", "eríamos", "iríamos",
			"ássemos", "êssemos", "íssemos", "aríeis", "eríeis", "iríeis", "ásseis", "ésseis", "ísseis", "áramos",
			"éramos", "íramos", "ávamos", "aremos", "eremos", "iremos", "ariam", "eriam", "iriam", "assem", "essem",
			"issem", "ara~o", "era~o", "ira~o", "arias", "erias", "irias", "ardes", "erdes", "irdes", "asses", "esses",
			"isses", "astes", "estes", "istes", "áreis", "areis", "éreis", "ereis", "íreis", "ireis", "áveis", "íamos",
			"armos", "ermos", "irmos", "aria", "eria", "iria", "asse", "esse", "isse", "aste", "este", "iste", "arei",
			"erei", "irei", "aram", "eram", "iram", "avam", "arem", "erem", "irem", "ando", "endo", "indo", "adas",
			"idas", "arás", "aras", "erás", "eras", "irás", "avas", "ares", "eres", "ires", "íeis", "ados", "idos",
			"ámos", "amos", "emos", "imos", "iras", "ada", "ida", "ará", "ara", "erá", "era", "irá", "ava", "iam",
			"ado", "ido", "ias", "ais", "eis", "ira", "ia", "ei", "am", "em", "ar", "er", "ir", "as", "es", "is", "eu",
			"iu", "ou");

	private final static SuffixTree<Boolean> suffixr = new SuffixTree<Boolean>(true, "os", "a", "i", "o", "á", "í",
			"ó");
	...
	private String step1(String st, String r1, String r2, String rv) {
		if (!(suffix = suffix1.getLongestSuffix(r2)).isEmpty()) {
			return st.substring(0, st.length() - suffix.length());
		}

		if (!(suffix = suffix2.getLongestSuffix(r2)).isEmpty()) {
			return st.substring(0, st.length() - suffix.length()) + "log";
		}

		if (!(suffix = suffix3.getLongestSuffix(r2)).isEmpty()) {
			return st.substring(0, st.length() - suffix.length()) + "u";
		}

		if (!(suffix = suffix4.getLongestSuffix(r2)).isEmpty()) {
	...
	private String step2(String st, String r1, String r2, String rv) {
		if (!(suffix = suffixv.getLongestSuffix(rv)).isEmpty()) {
			return st.substring(0, st.length() - suffix.length());
		}
		return st;
	}
	...
	private String step4(String st, String r1, String r2, String rv) {
		if (!(suffix = suffixr.getLongestSuffix(rv)).isEmpty()) {
			return st.substring(0, st.length() - suffix.length());
		}
		return st;
	}
