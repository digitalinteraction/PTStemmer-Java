...
public class OrengoStemmer extends Stemmer {
	...
	private String applyRules(String st, SuffixTree<Rule> rules) {
		...
		for (int i = res.size() - 1; i >= 0; i--) {
			...
			if (rules.getProperty("exceptions") == 1) { // Compare entire word
														// with exceptions
	...
	private void readRulesFromXML() throws PTStemmerException {
			...
			for (final Element rule : XMLUtils.getChilds(step)) {
				if (!rule.hasAttribute("size") || !rule.hasAttribute("replacement") || !rule.hasAttribute("suffix")) {
					throw new PTStemmerException(
							"Problem while parsing Orengo's XML stemming rules file: Invalid rule in " + stepName
									+ ".");
				}
				...
				for (final Element exception : XMLUtils.getChilds(rule)) {
		...
		if (pluralreductionrules == null || femininereductionrules == null || adverbreductionrules == null
				|| augmentativediminutivereductionrules == null || nounreductionrules == null
				|| verbreductionrules == null || vowelremovalrules == null) {
